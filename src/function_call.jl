...
function dispatch_typed_direct(fn::Union{Function, DataType}, canonical_args, mask)
	...
	#println("Potential fns $directly_callable, argtuple $argtuple")
	#println("Callable methods for argtuple $argtuple:")
	for mdef in directly_callable
		#println("$(mdef.method.sig); argtuple <: typ?: $(argtuple <: mdef.method.sig)")
	end
	#println("out of all methods")
	for mdef in methods(fn_rec)
		#println("$mdef.sig")
	end
	throw(InvalidMethodCall(fn_rec, canonical_args))
end