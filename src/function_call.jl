...
function dispatch_typed_direct(fn::Union{Function,DataType}, canonical_args, mask)
    ...
    if length(directly_callable) > 0 # there is a method that we can
        ...
        for mdef in directly_callable
            @debug("calling method on $mdef")
            @debug("is $argtuple <: $(mdef.method.sig)? $(argtuple <: mdef.method.sig) and args $(argtuple)")
            ...
            @debug("Callable $((mdef::Core.MethodMatch).method.sig), returning $rty, acc rty $rt")
        end
        if has_candidate
            return BasicType(rt)
        end
